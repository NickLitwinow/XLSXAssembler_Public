import sys
from PyQt5.QtWidgets import QApplication, QMainWindow, QPushButton, QVBoxLayout, QWidget, QFileDialog, QLabel, QListWidget, QMessageBox, QProgressBar
...
def combine_excel_sheets(file_paths, output_file, progress_callback):
    ...
    print(f'На вход дано: {len(file_paths)} файлов')
    print(f'Без ошибок считано: {len(file_paths)-len(missed_files)} файлов')
    print(f'Файлы которые считаны с ошибкой: {missed_files}')
    ...
    file_path = getattr(sys, '_MEIPASS', '.') + '/DEnew.xlsx'
...
class ExcelCombinerGUI(QMainWindow):
    ...
    def initUI(self):
        ...
        self.remove_file_button = QPushButton('Удалить файл')
        self.combine_button = QPushButton('Объединить файлы')
    ...
    def add_files(self):
        file_names, _ = QFileDialog.getOpenFileNames(self, 'Открыть файл', '', 'Excel Files (*.xlsx)')
    ...
    def combine_files(self):
        ...
        output_file, _ = QFileDialog.getSaveFileName(self, 'Сохранить файл', '', 'Excel Files (*.xlsx)')
        
        if not output_file:
            QMessageBox.warning(self, 'Ошибка', 'Необходимо указать файл для сохранения.')
            return

        try:
            combine_excel_sheets(file_paths, output_file, lambda progress: self.progress_bar.setValue(progress))
            self.status_label.setText('Файлы успешно объединены')
            QMessageBox.information(self, 'Готово', 'Файлы успешно объединены.')
        except Exception as e:
            QMessageBox.critical(self, 'Ошибка', f'Произошла ошибка при объединении файлов: {e}')
